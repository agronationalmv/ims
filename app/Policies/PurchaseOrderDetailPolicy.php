<?php

namespace App\Policies;

use Illuminate\Auth\Access\Response;
use App\Models\PurchaseOrderDetail;
use App\Models\User;

class PurchaseOrderDetailPolicy
{
    /**
     * Determine whether the user can view any models.
     */
    public function viewAny(User $user): bool
    {
        return $user->checkPermissionTo('view-any PurchaseOrderDetail');
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, PurchaseOrderDetail $purchaseorderdetail): bool
    {
        return $user->checkPermissionTo('view PurchaseOrderDetail');
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user): bool
    {
        return $user->checkPermissionTo('create PurchaseOrderDetail');
    }

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, PurchaseOrderDetail $purchaseorderdetail): bool
    {
        return $user->checkPermissionTo('update PurchaseOrderDetail');
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, PurchaseOrderDetail $purchaseorderdetail): bool
    {
        return $user->checkPermissionTo('delete PurchaseOrderDetail');
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, PurchaseOrderDetail $purchaseorderdetail): bool
    {
        return $user->checkPermissionTo('restore PurchaseOrderDetail');
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, PurchaseOrderDetail $purchaseorderdetail): bool
    {
        return $user->checkPermissionTo('force-delete PurchaseOrderDetail');
    }
}
